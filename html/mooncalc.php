...
/*
 This script is based on SunCalc a PHP library for calculating sun/moon position and light phases.
...
class SunCalc {
    ...
    function getMoonIllumination() {
        ...
        $phase_number = round(0.5 + 0.5 * $inc * ($angle < 0 ? -1 : 1) / PI,2,PHP_ROUND_HALF_DOWN);

    if ($phase_number > 0 && $phase_number < 0.05) {
        $phase_name = 'New Moon';
    } elseif ($phase_number > 0.051 && $phase_number < 0.25) {
        $phase_name = 'Waxing Crescent';
    } elseif ($phase_number === 0.25) {
        $phase_name = 'First Quarter';
    } elseif ($phase_number > 0.25 && $phase_number < 0.48) {
        $phase_name =  'Waxing Gibbous';
    } elseif ($phase_number > 0.481 && $phase_number < 0.529) {
        $phase_name =  'Full Moon';
    } elseif ($phase_number > 0.53 && $phase_number < 0.75) {
        $phase_name =  'Waning Gibbous';
    } elseif ($phase_number === 0.75) {
        ...
        return [
            'fraction' => round((1 + cos($inc)) / 2 * 100),
...
function getMoonTimes($inUTC=false) {
    ...
    // Track if moonrise occurs after midnight
    $moonriseAfterMidnight = false;

    // Loop over two days
    for ($dayOffset = 0; $dayOffset <= 1; $dayOffset++) {
        // go in 2-hour chunks, each time seeing if a 3-point quadratic curve crosses zero (which means rise or set)
        for ($i = 1; $i <= 24; $i += 2) {
            $h1 = $this->getMoonPosition(hoursLater($t, $i + $dayOffset * 24))->altitude - $hc;
            $h2 = $this->getMoonPosition(hoursLater($t, $i + 1 + $dayOffset * 24))->altitude - $hc;
            ...
            if ($roots === 1) {
                if ($h0 < 0) $rise = $i + $x1 + $dayOffset * 24;
                else $set = $i + $x1 + $dayOffset * 24;

            } else if ($roots === 2) {
                $rise = $i + ($ye < 0 ? $x2 : $x1) + $dayOffset * 24;
                $set = $i + ($ye < 0 ? $x1 : $x2) + $dayOffset * 24;
            }
    ...
    if ($rise != 0) {
        // Handle moonrise that happens after midnight (next day)
        if ($rise >= 24) {
            $moonriseAfterMidnight = true;
            $rise -= 24;
            $t->modify('+1 day');
        }
...
// Collect results
$sunTimes = $test->getSunTimes();
$moonIllumination = $test->getMoonIllumination();
$moonTimes = $test->getMoonTimes();

// Convert DateTime objects to strings without timezone info
foreach ($sunTimes as $key => $value) {
    if ($value instanceof \DateTime) {
        $sunTimes[$key] = $value->format(DATE_ISO8601); // or use a custom format like 'Y-m-d H:i:s'
    }
}

foreach ($moonTimes as $key => $value) {
    if ($value instanceof \DateTime) {
        $moonTimes[$key] = $value->format('H:i'); // or use a custom format
    }
}
...
// $combinedResult = array_merge($sunTimes,$moonIllumination, $moonTimes);
